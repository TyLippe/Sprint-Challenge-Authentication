...
Sessions let us keep the authenication active so a user can make requests without being asked to log back in everytime.
...
As I mentioned it will hash out the password, when we do this is shows the number of times it has gone through the hash process, when an attacker sees the number it will let them know this is not the site to attempt to get into, because it will take a very long time!
